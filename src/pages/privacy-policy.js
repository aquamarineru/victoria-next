...
const serializers = {
  ...
  const PrivacyPolicy = ({ policy, locale }) => {
    console.log(policy);
    const localizedTitle = policy.title?.find(item => item._key === locale)?.value;
    const localizedBackButton = policy.backButton?.find(item => item._key === locale)?.value || 'Back';
  ...
  export async function getStaticProps({ locale }) {
    try {
        ...
        const [policyData] = await client.fetch(query); // Updated here

        return {
            props: {
                policy: policyData,  // Updated here
                locale
            }
        };
    } catch (err) {
